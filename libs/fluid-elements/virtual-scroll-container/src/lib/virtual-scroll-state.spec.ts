...
import {
  FluidVirtualScrollRenderedItemRange,
  VirtualScrollState,
} from './virtual-scroll-state';
...
function checkScrollState(
  ...
  renderedItemsRange: FluidVirtualScrollRenderedItemRange,
  ...
  expect(instance.renderedItemsRange).toEqual(renderedItemsRange);
...
describe(`Fluid virtual scroll state`, () => {
  ...
  it(`should create the scroll state instance`, () => {
    ...
    checkScrollState(instance, { first: -1, last: -1 }, 0, 0, 0, 0, -1, -1);
  });

  it(`should not update items range as long as items count is 0`, () => {
    instance.addInitialItemsBatch();
    expect(instance.renderedItemsRange).toEqual({
      first: -1,
      last: -1,
    });
  });
  ...
  describe(`no need to scroll due to too little items`, () => {
    ...
    beforeEach(() => {
      resetSpy = jest.spyOn(instance, `reset`);
      instance.itemsCount = 3;
    });

    it(`should reset the virtual scroll state when the items count changes`, () => {
      expect(resetSpy).toHaveBeenCalledTimes(1);
      expect(instance.avgItemHeight).toEqual(0);
      checkScrollState(instance, { first: -1, last: -1 }, 0, 0, 0, 0, -1, -1);
    });

    it(`should not add more items than available`, () => {
      instance.addInitialItemsBatch();
      checkScrollState(instance, { first: 0, last: 2 }, 0, 0, 0, 0, -1, -1);

      instance.addInitialItemsBatch();
      instance.addInitialItemsBatch();
      instance.updateEndScrollBuffer();
      checkScrollState(instance, { first: 0, last: 2 }, 0, 0, 0, 0, -1, -1);
    });
  });
  ...
  describe(`items count = 20`, () => {
    ...
    it(`should add one batch of items`, () => {
      instance.addInitialItemsBatch();
      checkScrollState(instance, { first: 0, last: 4 }, 0, 0, 0, 0, -1, -1);
    });

    it(`should add two batches of items`, () => {
      instance.addInitialItemsBatch();
      instance.addInitialItemsBatch();
      checkScrollState(instance, { first: 0, last: 9 }, 0, 0, 0, 0, -1, -1);
    });

    it(`should add items plus buffer`, () => {
      instance.addInitialItemsBatch();
      instance.updateEndScrollBuffer();
      checkScrollState(instance, { first: 0, last: 7 }, 0, 0, 0, 3, -1, 6);
    });
    ...
    it(`should not remove items at start, if there is no buffer yet`, () => {
      ...
      instance.updateEndScrollBuffer();
      instance.updateEndScrollBuffer();
      instance.calculatePlaceholderSize();
      checkScrollState(instance, { first: 0, last: 10 }, 0, 180, 3, 3, 1, 9);
    });
    ...
    it(`should remove items at the start`, () => {
      ...
      instance.updateEndScrollBuffer();
      instance.calculatePlaceholderSize();
      checkScrollState(instance, { first: 3, last: 13 }, 60, 120, 3, 3, 4, 12);
    });
  });
  ...
  describe(`initialise virtual scroll state with options`, () => {
    ...
    it(`should not update items range as long as items count is 0`, () => {
      instance.addInitialItemsBatch();
      expect(instance.renderedItemsRange).toEqual({ first: -1, last: -1 });
    });
    ...
    describe(`items count = 50`, () => {
      ...
      it(`should add two batches of items`, () => {
        instance.addInitialItemsBatch();
        instance.addInitialItemsBatch();
        checkScrollState(instance, { first: 0, last: 19 }, 0, 0, 0, 0, -1, -1);
      });

      it(`should add items plus buffer`, () => {
        instance.addInitialItemsBatch();
        instance.updateEndScrollBuffer();
        checkScrollState(instance, { first: 0, last: 14 }, 0, 0, 0, 5, -1, 12);
      });
      ...
      it(`should not remove items at start, if there is no buffer yet`, () => {
        ...
        instance.updateEndScrollBuffer();
        instance.calculatePlaceholderSize();
        checkScrollState(instance, { first: 0, last: 19 }, 0, 600, 5, 5, 2, 17);
      });
