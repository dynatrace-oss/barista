...
import {
  ...
  virtualScrollUpdateStartScrollBuffer,
...
export class FluidVirtualScrollContainer<T> extends LitElement {
  ...
  /**
   * Defines the max height of the virtual scroll container
   * If this is not set, a parent of the virtual scroll container should
   * have an explicit height, else it will grow until all items are rendered
   * @attr
   */
  @property({ type: String, reflect: true })
  maxheight: string;
  ...
  _scrollState = new FluidVirtualScrollState();
  ...
  /**
   * The scroll container might not be visible when being first rendered
   * Unless it is visible, no items should be rendered
   */
  private _visibilityObserver: IntersectionObserver;

  /** Defines whether the virtual scroll container is visible */
  ...
  private _resetVirtualScroll(): void {
    ...
    this._scrollState = virtualScrollResetState(this._scrollState);
  ...
  private _renderInitialVirtualScrollItems(): void {
    ...
    this._scrollState = virtualScrollAddInitialItemsBatch(this._scrollState);
    this._renderVirtualScrollItems();

    // Only check if the initial render is complete after the browser
    // actually rendered the first batch of virtual scroll items
    requestAnimationFrame(() => {
      if (!this._isInitialRenderDone()) {
        ...
        this._scrollState.renderedItemsRangeEnd !==
        this.items.length - 1
      ) {
        // Call scroll state update end buffer to add the buffer items
        this._scrollState = virtualScrollUpdateEndScrollBuffer(
          this._scrollState,
        );
        ...
        let totalHeight = 0;
        for (
          let i = this._scrollState.renderedItemsRangeStart;
          i <= this._scrollState.renderedItemsRangeEnd;
          i += 1
        ) {
        ...
        this._scrollState.avgItemHeight =
          totalHeight /
          (this._scrollState.renderedItemsRangeEnd -
            this._scrollState.renderedItemsRangeStart +
            1);
  ...
  private _handleHostIntersection(
  ...
  private _handlePaddingIntersection(
    ...
    observedItems.forEach((entry) => {
      ...
      /*
       * TODO
       * figure out a way to continuosly render items, even when scrolling
       * very fast.
       */
      ...
      if (!this._observedItemsIndices.has(itemIndex)) {
        if (
          ...
          this._clearVirtualScrollBufferObserver();
          this._scrollState = virtualScrollUpdateStartScrollBuffer(
            this._scrollState,
          );
          this._renderVirtualScrollItems();
          this._updatePlaceholders();
      ...
      if (entry.isIntersecting) {
        ...
        this._scrollState =
          itemIndex === this._scrollState.bufferStartObservedItemIndex
            ? virtualScrollUpdateStartScrollBuffer(this._scrollState)
            : virtualScrollUpdateEndScrollBuffer(this._scrollState);
  ...
  private _isInitialRenderDone(): boolean {
    if (
      ...
      this._scrollState.renderedItemsRangeEnd === this.items.length - 1
    ) {
  ...
  firstUpdated(props: PropertyValues): void {
    ...
    this._visibilityObserver = new IntersectionObserver(
      this._handleHostIntersection.bind(this),
    );
  ...
  render(): TemplateResult {
    if (this._isVisible && this._scrollContainer) {
    ...
    return html`
      <div
        ...
        class="fluid-virtual-scroll-container"
        style="max-height: ${ifDefined(this.maxheight)};"
      >
