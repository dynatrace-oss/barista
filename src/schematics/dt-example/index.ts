 ...
 */
function addExampleInFile(options: DtExampleOptions, file: string, syntaxKind: ts.SyntaxKind): Rule {
    return (host: Tree) => {
        if(syntaxKind!==ts.SyntaxKind.VariableDeclaration && syntaxKind!==ts.SyntaxKind.PropertyDeclaration){
            throw new SchematicsException(`Invalid ts.SyntaxKind.`);
        }
        ...
        const importLocation = `'./examples/${strings.dasherize(options.name)}-${strings.dasherize(options.component)}-example.component';`;
        ...
        const toInsert = (file==="component") ? `${indentation}${strings.classify(options.name)}: ${importName},` : `${indentation}${importName},`;
...
export default function(options: DtExampleOptions): Rule {
    ...
    const moduleFile = fs.existsSync(path.join('src', 'docs', 'components', options.component, `docs-${options.component}.module.ts`));
    const componentFile = fs.existsSync(path.join('src', 'docs', 'components', options.component, `docs-${options.component}.component.ts`));
    const htmlFile = fs.existsSync(path.join('src', 'docs', 'components',options.component, `docs-${options.component}.component.html`));
