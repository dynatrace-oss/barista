...
describe('DtMicroChart - Core', () => {
  ...
  it('should unify the input data into a number[][]', () => {
    ...
    expect(newSeries[0]).toEqual([0, 0]);
    expect(newSeries[1]).toEqual([1, 15]);
    expect(newSeries[2]).toEqual([2, 187]);
    expect(newSeries[3]).toEqual([3, 182]);
  });
  ...
  it('should unify input data when it contains null values', () => {
    ...
    expect(newSeries[0]).toEqual([0, 0]);
    expect(newSeries[1]).toEqual([1, 15]);
    expect(newSeries[2]).toEqual([2, null]);
    expect(newSeries[3]).toEqual([3, 182]);
  });
  ...
  it('should not modifiy the array if input data is already number[][]', () => {
    ...
    const newSeries = unifySeriesData(simpleLineSeries);
    expect(newSeries[0]).toEqual([0, 0]);
    expect(newSeries[1]).toEqual([10, 15]);
    expect(newSeries[2]).toEqual([27, 187]);
    expect(newSeries[3]).toEqual([162, 182]);
  });
});
