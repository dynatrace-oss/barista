...
@Component({
  ...
  <dt-row *dtRowDef="let row; columns: ['host', 'cpu', 'memory', 'traffic']; let rowIndex=index; when: !isNarrow"></dt-row>
  <dt-expandable-row *dtRowDef="let row; columns: ['host', 'cpu', 'details']; let rowIndex=index; when: isNarrow">
    <dt-key-value-list>
...
export class TableResponsiveExample {
  dataSource: HostMetricResponsive[] = [
    { host: 'et-demo-2-win4', cpu: '30 %', memory: '38 % of 5.83 GB', traffic: '98.7 Mbit/s', isNarrow: false },
    { host: 'et-demo-2-win3', cpu: '26 %', memory: '46 % of 6 GB', traffic: '625 Mbit/s', isNarrow: false },
    { host: 'docker-host2', cpu: '25.4 %', memory: '38 % of 5.83 GB', traffic: '419 Mbit/s', isNarrow: false },
    { host: 'et-demo-2-win1', cpu: '23 %', memory: '7.86 % of 5.83 GB', traffic: '98.7 Mbit/s', isNarrow: false },
  ];
  ...
  constructor(
    ...
    this._viewportResizer.change().pipe(
      startWith(null),
      switchMap(() => this._zone.onStable.pipe(take(1))))
    .subscribe(() => {
      const narrow = this._viewportRuler.getViewportSize().width < NARROW_THRESHHOLD ? true : false;
      if (this.dataSource[0].isNarrow !== narrow) {
        this.dataSource = this.dataSource.map((data) => { data.isNarrow = narrow; return data; });
      }
      if (narrow) {
