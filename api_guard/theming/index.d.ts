export declare const DT_CHART_COLOR_PALETTE_ORDERED: string[];

export declare const DT_CHART_COLOR_PALETTES: {
    purple: string[];
    royalblue: string[];
    blue: string[];
};

export declare enum DtColors {
    WHITE = "#ffffff",
    BLUE_100 = "#e0f4fc",
    BLUE_200 = "#b4e5f9",
    BLUE_300 = "#74cff7",
    BLUE_400 = "#2ab6f4",
    BLUE_500 = "#14a8f5",
    BLUE_600 = "#008cdb",
    BLUE_700 = "#006bba",
    BLUE_800 = "#004999",
    BLUE_900 = "#003a8a",
    PURPLE_100 = "#eedbfd",
    PURPLE_200 = "#debbf3",
    PURPLE_300 = "#c396e0",
    PURPLE_400 = "#a972cc",
    PURPLE_500 = "#9355b7",
    PURPLE_600 = "#7c38a1",
    PURPLE_700 = "#612c85",
    PURPLE_800 = "#522273",
    PURPLE_900 = "#471e64",
    ROYALBLUE_100 = "#e6eaff",
    ROYALBLUE_200 = "#b9c5ff",
    ROYALBLUE_300 = "#97a9ff",
    ROYALBLUE_400 = "#748cff",
    ROYALBLUE_500 = "#526cff",
    ROYALBLUE_600 = "#4556d7",
    ROYALBLUE_700 = "#393db0",
    ROYALBLUE_800 = "#31339c",
    ROYALBLUE_900 = "#231874",
    TURQUOISE_100 = "#e1f7f9",
    TURQUOISE_200 = "#aeebf0",
    TURQUOISE_300 = "#74dee6",
    TURQUOISE_400 = "#4fd5e0",
    TURQUOISE_500 = "#00b9cc",
    TURQUOISE_600 = "#00a1b2",
    TURQUOISE_700 = "#00848e",
    TURQUOISE_800 = "#006d75",
    TURQUOISE_900 = "#005559",
    YELLOW_100 = "#fff9d5",
    YELLOW_200 = "#fff29a",
    YELLOW_300 = "#ffee7c",
    YELLOW_400 = "#ffe11c",
    YELLOW_500 = "#f5d30f",
    YELLOW_600 = "#e6be00",
    YELLOW_700 = "#c9a000",
    YELLOW_800 = "#ab8300",
    YELLOW_900 = "#8e6500",
    ORANGE_100 = "#fff5e4",
    ORANGE_200 = "#ffd0ab",
    ORANGE_300 = "#ffa86c",
    ORANGE_400 = "#fd8232",
    ORANGE_500 = "#ef651f",
    ORANGE_600 = "#c95218",
    ORANGE_700 = "#b64915",
    ORANGE_800 = "#8d380f",
    ORANGE_900 = "#742d0b",
    RED_100 = "#ffeaea",
    RED_200 = "#f9acaf",
    RED_300 = "#f28289",
    RED_400 = "#e74d59",
    RED_500 = "#dc172a",
    RED_600 = "#c41425",
    RED_700 = "#ab0c17",
    RED_800 = "#93060e",
    RED_900 = "#7a0004",
    GREEN_100 = "#e8f9dc",
    GREEN_200 = "#d2efbe",
    GREEN_300 = "#bee5a3",
    GREEN_400 = "#9cd575",
    GREEN_500 = "#7dc540",
    GREEN_600 = "#5ead35",
    GREEN_700 = "#3f962a",
    GREEN_800 = "#1f7e1e",
    GREEN_900 = "#006613",
    SHAMROCKGREEN_100 = "#e1f7dc",
    SHAMROCKGREEN_200 = "#bbeac0",
    SHAMROCKGREEN_300 = "#99dea8",
    SHAMROCKGREEN_400 = "#6bcb8b",
    SHAMROCKGREEN_500 = "#54c27d",
    SHAMROCKGREEN_600 = "#2ab06f",
    SHAMROCKGREEN_700 = "#009e60",
    SHAMROCKGREEN_800 = "#048855",
    SHAMROCKGREEN_900 = "#087249",
    GRAY_100 = "#f8f8f8",
    GRAY_130 = "#f2f2f2",
    GRAY_200 = "#e6e6e6",
    GRAY_300 = "#cccccc",
    GRAY_400 = "#b7b7b7",
    GRAY_500 = "#898989",
    GRAY_600 = "#6d6d6d",
    GRAY_640 = "#525252",
    GRAY_700 = "#454646",
    GRAY_800 = "#353535",
    GRAY_860 = "#242424",
    GRAY_900 = "#191919"
}

export declare class DtTheme implements OnDestroy {
    get _depthLevel(): number;
    readonly _stateChanges: Subject<void>;
    set dtTheme(value: string);
    get name(): string | null;
    get variant(): DtThemeVariant | null;
    constructor(_elementRef: ElementRef, _parentTheme: DtTheme);
    ngOnDestroy(): void;
}

export declare type DtThemeVariant = 'light' | 'dark' | null;

export declare class DtThemingModule {
}

export declare function getDtChartColorPalette(nrOfMetrics: number, theme?: DtTheme): string[];
